...
void rm_test(void){
  // tests if files are deleted correctly, as well as when a non-existent file is called to be removed. 
  ...
  fclose(t1);
  retval = beargit_add("t1.txt");
  CU_ASSERT(0 == retval);
  FILE* t2 = fopen("t2.txt", "w");
  fclose(t2);
  retval = beargit_add("t2.txt");
  CU_ASSERT(0 == retval);
  FILE* t3 = fopen("t3.txt", "w");
  fclose(t3);
  retval = beargit_add("t3.txt");
  CU_ASSERT(0 == retval);
  retval = beargit_rm("t2.txt");
  CU_ASSERT(1==retval);
  retval = beargit_rm("asdf.txt");
  CU_ASSERT(0==retval);
  char x[100];
  read_string_from_file("TEST_STDERR", x, 100);
  char* partial_err_string = "ERROR: File";
  CU_ASSERT(0 == strncmp(x,partial_err_string,strlen(partial_err_string)));


  FILE* t4 = fopen("t2.txt", "w");
  fclose(t4);
  retval = beargit_add("t2.txt");
  CU_ASSERT(0==retval);
...
void add_test(void) { // tests if files are added correctly, and errors when we add the same file. 
  int retval;
  retval = beargit_init();
  CU_ASSERT(0==retval);
  FILE* t1 = fopen("t1.txt", "w");
...
void commit_test(void) {
  int retval;
  retval = beargit_commit("no bears");
  CU_ASSERT(retval == 1 );
  retval = beargit_commit("GO BEARS! 123");
  CU_ASSERT(retval ==0);
  char x[100];
  retval = beargit_commit("GO BEARS! 1234");
  CU_ASSERT(0 == retval);
  read_string_from_file("TEST_STDERR", x, 100);
  char* partial_err_string = "ERROR: Need to be on HEAD";
  CU_ASSERT(0 == strncmp(x,partial_err_string,strlen(partial_err_string)));
...
int cunittester()
{
  ...
  pSuite3 = CU_add_suite("Suite_3", init_suite, clean_suite);
   ...
   if (NULL == CU_add_test(pSuite3, "add_test ", add_test))// tests if files are added correctly, and errors when we add the same file. 
   {
      CU_cleanup_registry();
      return CU_get_error();
   }
   ...
   if (NULL == CU_add_test(pSuite4, "rm_test", rm_test)) // tests if files are deleted correctly, as well as when a non-existent file is called to be removed. 
   {
      CU_cleanup_registry();
      return CU_get_error();
   }
