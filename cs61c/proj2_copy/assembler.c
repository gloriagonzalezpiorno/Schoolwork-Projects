...
int pass_one(FILE* input, FILE* output, SymbolTable* symtbl) {
    ...
    while(fgets(buf, BUF_SIZE, input)) {
        ...
        while(token!=NULL){
            if(label_counter == 0){
                int check_label = add_if_label(input_line, token, byte_offset, symtbl);
                if(check_label ==0)
                {
                    count++;
                    if(count==1){
        ...
        if(error != -1){
            if(cmd!= NULL){
                unsigned int lines_written = write_pass_one(output, cmd, args, num_args);
            if (lines_written == 0) {
                raise_inst_error(input_line, cmd, args, num_args);
                ret_code = -1;
        } 
...
int pass_two(FILE *input, FILE* output, SymbolTable* symtbl, SymbolTable* reltbl) {
    ...
    while(fgets(buf, BUF_SIZE,input)){
        ...
        int t_inst = translate_inst(output, cmd, args, num_args, byte_offset, symtbl, reltbl);
