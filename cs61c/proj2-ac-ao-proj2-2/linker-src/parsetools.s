...
hex_to_str:
	...
	li $t1 0 
	li $t2 0 # get ready 
	addiu $t0 $0 32 #counter
	move $t2 $s1 	# char buffer 

hex_loop:	
	beq $t0 $0 hex_exit
	addiu $t0 $t0 -4 #move down t0 counter by 4 
	srlv $t1 $s0 $t0 #shift right by t0 , into $t1
	andi $t1 $t1 0xF #take the 4 right bits into $t1
	# need to convert to ascii
	bgt $t1 $t3 greater_eq_10 # need to return to here 
	j NOT_greater_eq_10

rest:
	sb $t1 0($t2) # save it into buffer # warning: $t1 is still an int, need to save as char? 
	addiu $t2 $t2 1 # move buffer address by 1 
	j hex_loop	#do again 
...
hex_exit:
	addiu $t1 $0 10 #new line 
	sb $t1 0($t2)
	addiu $t2 $t2 1 

	addiu $t1 $0 0 #null terminator 
	sb $t1 0($t2)
