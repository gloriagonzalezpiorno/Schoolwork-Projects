 ...
 */
   ...
   /*
    ...
    str
      string containing the representation of an integral number.

    endptr
      This is the reference to an object of type char*, 
      whose value is set by the function to the next character 
      in str after the numerical value.

    base
      This is the base, which must be between 2 and 36 inclusive, 
...
int translate_num(long int* output, const char* str, long int lower_bound, 
    ...
    char* endptr;
    long int str_int_long = strtol(str, &endptr, 0); // base of 0 : special key
    // 0 cases
    if (*str == '0' || strcmp(str, "0")==0 || strcmp(str, "0x0")==0 ) {
      *output = str_int_long;
      return 0;
    }
    // end ptr == null, no more string
    if (*endptr == '\0') {
      // non zero
      if (str_int_long != 0L) {
        // right range
        if (str_int_long >= lower_bound && str_int_long <= upper_bound) {
          *output = str_int_long;
          return 0;
        } 
      } 
