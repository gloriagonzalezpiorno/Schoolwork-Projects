...
unsigned write_pass_one(FILE* output, const char* name, char** args, int num_args) {
    ...
    if (strcmp(name, "li") == 0) { // number of arguments should be two 
      ...
      if (num_args != 2) { 
        ...
        int t_num = translate_num(&immd, args[1], -2147483648, 4294967295);
        if (t_num == -1) {
          return 0;
        }
        if (is_signed_int(immd) || is_unsigned_int(immd)) {
          ...
          fprintf(output, "lui %s %ld\n", "$at", get_high_sixteen(immd));
          fprintf(output, "ori %s %s %ld\n", args[0], "$at", get_low_sixteen(immd));
          return 2;
        }        
      }
...
int write_branch(uint8_t opcode, FILE* output, char** args, size_t num_args, uint32_t addr, SymbolTable* symtbl) {
    ...
    int32_t address = label_addr - addr;
    int branchable = can_branch_to(addr, label_addr);
    if (branchable) {
      // if (address % BYTE == 0) {
        address = ( (address -4) / 4) & 0x0000FFFF;  // a / 4
      // }
    }
