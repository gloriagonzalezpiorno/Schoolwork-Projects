...
def eval_all(expressions, env, rec = True):
    ...
    return scheme_eval(expressions.first, env, rec)
...
class Frame(object):
    ...
    def make_child_frame(self, formals, vals):
        ...
        # BEGIN Question 10
        def checklength(formals, vals):
            if formals is nil and vals is nil:
                return True
            if formals is nil and vals != nil:
                return False
            if formals != nil and vals is nil:
                return False
            return checklength(formals.second, vals.second)

        if checklength(formals, vals) is False:
            raise SchemeError("Too many or too few")
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        lambda_exp = Pair("lambda",args)
        target_new = expressions.first.first
        env.bindings[target_new] = scheme_eval(lambda_exp,env)
        return target_new
        # END Question 9A
    else:
...
def do_if_form(expressions, env):
    ...
    output = expressions.second

    test = scheme_eval(cond, env)
    if test is True:
        return scheme_eval(output.first, env, True)
    elif test is False:
        if output.second is nil:
            return okay
        return scheme_eval(output.second.first, env, True)
    elif test is not nil:
        return scheme_eval(output.first, env, True)
    else:
        try:
            return vals[2]
...
def do_and_form(expressions, env):
    ...
    while expressions is not nil:
        ...
        if x==False and expressions.first is not 0:
            return False
...
def make_let_frame(bindings, env):
    ...
    while bindings is not nil:
        if checkarg(bindings.first):
            raise SchemeError("Bad args")

        temp = bindings.first.first
        if not isinstance(temp, str):
            raise SchemeError("Not string")
        formals = Pair(temp, formals)
        temp = scheme_eval(bindings.first.second.first, env)
        vals = Pair(temp,vals)
        bindings = bindings.second
       
      
    frame = env.make_child_frame(formals, vals) 
    return frame
    # END Question 16
############################ Stream ##########################
# class Promise:
#     def __init__(self, expr, env, forced = False):
#         self.expr = expr
#         self.env = env
#         self.forced = forced
...
#         else:
#             temp = Frame(self.env)
#             self.expr = eval_all(self.expr, temp)
#             self.forced = True
#             return self.expr
...
#     return Pair(first, do_delay_form(expressions, env))
...
class Promise:
    ...
    def evaluate(self):
        if self.forced:
            ...
            self.expr = eval_all(self.expr, frame, False)
            self.forced = True
        return self.expr
...
def check_formals(formals):
    ...
    temp = [] 
    while formals is not nil:
        temp = temp + [formals.first]
        if not scheme_symbolp(formals.first):
            raise SchemeError("List of symbols not well-formed")
        formals = formals.second
    if len(temp) != len(set(temp)):
        raise SchemeError("Found repeated symbols")
