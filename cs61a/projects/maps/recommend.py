...
from utils import distance, mean, zip, enumerate, sample,map_and_filter
...
def group_by_centroid(restaurants, centroids):
    ...
    for x in restaurants:
        lst = lst + [[find_closest(restaurant_location(x), centroids),x]]
...
def find_centroid(cluster):
    ...
    loc = [] 
    for x in cluster:
        loc = loc +[restaurant_location(x)]
    
    x = mean([a[0] for a in loc])
    y = mean([a[1] for a in loc])
    return [x,y]
    # END Question 5
...
def best_predictor(user, restaurants, feature_fns):
    ...
    for x in feature_fns:
        res = res + [find_predictor(user,reviewed,x)]
...
def rate_all(user, restaurants, feature_fns):
    ...
    for x in restaurants:
        ...
        if x in reviewed:
            name = restaurant_name(x)
            temp = user_rating(user,name)
