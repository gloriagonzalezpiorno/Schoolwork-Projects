...
class Place(object):
    ...
    def add_insect(self, insect):
        ...
        if insect.is_ant:
            ...
            if self.ant != None:
                ...
                if self.ant.can_contain(insect) == False and insect.can_contain(self.ant) == False:
            ...
            elif self.ant.can_contain(insect) == True:
    ...
    def remove_insect(self, insect):
        ...
        if insect.is_ant:
            ...
            # BEGIN Problem 7
            if type(insect) == QueenAnt and not insect.fake_queens:
                return
            elif type(insect) == BodyguardAnt or type(insect) == TankAnt:
                if insect.ant != None:
                    self.ant = insect.ant
                else:
                    self.ant = None
            elif type(self.ant) == BodyguardAnt or type(self.ant) == TankAnt:
                if self.ant.ant == insect:
                    self.ant.ant = None
            else:
                self.ant = None

            '''if type(insect) == BodyguardAnt or type(insect) == TankAnt:
            	if insect.ant != None:
            		self.ant = insect.ant
            	else:
            		self.ant = None
                ...
                if type(self.ant) == BodyguardAnt or type(self.ant) == TankAnt:
            	    if self.ant.ant == insect:
            		    self.ant.ant = None 
            # self.ant = None
            if type(insect) == QueenAnt and not insect.fake_queens:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        index = 0
        while x != hive:
            if x.bees != [] and index >= self.min_range and index <= self.max_range:
...
class QueenAnt(ScubaThrower):  # You should change this line
    ...
    def action(self, colony):
        ...
        """
        # BEGIN Problem 9
        if self.fake_queens == True:
            self.reduce_armor(self.armor)
        else:
            ScubaThrower.action(self, colony)
            next_place = self.place.exit

            if self.num_colony == 0:
                colony.queen = QueenPlace(self.place,colony.queen)
                self.num_colony += 1

            while next_place != None:
                next_ant = next_place.ant
                if next_ant != None:
                    if not next_ant in self.ant_double_list:
                        if type(next_ant) == BodyguardAnt:
                            next_ant.damage *= 2
                        else:
                            next_ant.damage *= 2
                        self.ant_double_list.append(next_ant)
                if type(next_ant) == BodyguardAnt or type(next_ant) == TankAnt:
                    if next_ant.ant != None and not next_ant.ant in self.ant_double_list:
                        next_ant.ant.damage *= 2
                        self.ant_double_list.append(next_ant.ant)


                if type(self.place.ant) == BodyguardAnt or type(self.place.ant) == TankAnt:
                    if type(self.place.ant.ant) == QueenAnt and not self.place.ant in self.ant_double_list:
                        self.place.ant.damage *= 2
                        self.ant_double_list.append(self.place.ant)

                    if type(self.place.ant.ant) != QueenAnt and not self.place.ant.ant in self.ant_double_list:
                        self.place.ant.ant.damage *= 2
                        self.ant_double_list.append(self.place.ant.ant)
