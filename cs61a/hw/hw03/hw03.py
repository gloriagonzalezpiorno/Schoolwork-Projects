
better_keep_if = 1  # REPLACE None WITH 1 or 2

better_keep_if_explanation = """
The else statement is completely redundant in #2, makes it more confusing. 
"""


def g(n):
    """Return the value of G(n), computed recursively.

    >>> g(1)
    1
    >>> g(2)
    2
    >>> g(3)
    3
    >>> g(4)
    10
    >>> g(5)
    22
    """
    if (n <= 3 ):
    	return n
    else:
    	return g(n-1) + 2*g(n-2) + 3*g(n-3)

def g_iter(n):
    """Return the value of G(n), computed iteratively.

    >>> g_iter(1)
    1
    >>> g_iter(2)
    2
    >>> g_iter(3)
    3
    >>> g_iter(4)
    10
    >>> g_iter(5)
    22
    """
    g1 = 1 
    g2 = 2 
    g3 = 3 
    if(n <= 3):
    	return n
    else:
    	x = 3
    	while ( x!= n):
    		x = x+1
    		temp = g1
    		g1= g2
    		g2 = g3
    		g3 = g2 + 2*g1 + 3*temp 
    return g3
def pingpong(n):
    """Return the nth element of the ping-pong sequence.

    >>> pingpong(7)
    7
    >>> pingpong(8)
    6
    >>> pingpong(15)
    1
    >>> pingpong(21)
    -1
    >>> pingpong(22)
    0
    >>> pingpong(30)
    6
    >>> pingpong(68)
    2
    >>> pingpong(69)
    1
    >>> pingpong(70)
    0
    >>> pingpong(71)
    1
    >>> pingpong(72)
    0
    >>> pingpong(100)
    2
    """
   
    def jump(diff, x, y):
        if(n==x):
            return y
        if(x!=0):
            if(x%7 ==0 or has_seven(x)):
                return jump(-1*diff, x+1, y - diff)
        return jump(diff, x+1, y+diff)
    return jump(1,0,0)

    
    if(n < 7):
    	return n
    if ((n-1) % 7 == 0 or has_seven(n-1)):
    	
    	return pingpong(n-2)

    else:
    	return 2*pingpong(n-1) - pingpong(n-2) 


    
def has_seven(k):
    """Returns True if at least one of the digits of k is a 7, False otherwise.

    >>> has_seven(3)
    False
    >>> has_seven(7)
    True
    >>> has_seven(2734)
    True
    >>> has_seven(2634)
    False
    >>> has_seven(734)
    True
    >>> has_seven(7777)
    True
    """
    if k % 10 == 7:
        return True
    elif k < 10:
        return False
    else:
        return has_seven(k // 10)


def count_change(amount):
    """Return the number of ways to make change for amount.

    >>> count_change(7)
    6
    >>> count_change(10)
    14
    >>> count_change(20)
    60
    >>> count_change(100)
    9828
    """
    def partition_change(a, maxi):
        if(a==0):
            return 1
        elif(a<0 or maxi==0):
            return 0
        else:
            with_max = partition_change(a - maxi, maxi)
            without_max = partition_change(a, maxi//2)
            return with_max + without_max
    temp = 1
    while (temp <= amount):
        temp = 2*temp
    return partition_change(amount, temp)

def towers_of_hanoi(n, start, end):
    """Print the moves required to solve the towers of hanoi game, starting
    with n disks on the start pole and finishing on the end pole.

    The game is to assumed to have 3 poles.

    >>> towers_of_hanoi(1, 1, 3)
    Move the top disk from rod 1 to rod 3
    >>> towers_of_hanoi(2, 1, 3)
    Move the top disk from rod 1 to rod 2
    Move the top disk from rod 1 to rod 3
    Move the top disk from rod 2 to rod 3
    >>> towers_of_hanoi(3, 1, 3)
    Move the top disk from rod 1 to rod 3
    Move the top disk from rod 1 to rod 2
    Move the top disk from rod 3 to rod 2
    Move the top disk from rod 1 to rod 3
    Move the top disk from rod 2 to rod 1
    Move the top disk from rod 2 to rod 3
    Move the top disk from rod 1 to rod 3
    """
    assert 0 < start <= 3 and 0 < end <= 3 and start != end, "Bad start/end"
    "*** YOUR CODE HERE ***"
    if(n==1):
    	print("Move the top disk from rod " + str(start) +" to rod " + str(end))
    else:
        other = 0
        for x in range(1,4):
            if(x!=start and x!= end ):
                other = x
        towers_of_hanoi(n-1, start, other)
        print("Move the top disk from rod " + str(start) +" to rod " + str(end))
        towers_of_hanoi(n-1, other, end)


from operator import sub, mul

def make_anonymous_factorial():
    """Return the value of an expression that computes factorial.

    >>> make_anonymous_factorial()(5)
    120
    """
    return 'YOUR_EXPRESSION_HERE'

